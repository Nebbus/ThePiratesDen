...
[CustomEditor(typeof(MONO_DataResetter))]
public class EDI_DataResetter : Editor
{
    private MONO_DataResetter dataResetter;         // Reference to the target of this Editor.
    ...
    private const string dataResetterPropResettableScriptableObjectsName = "SOBJ_resettableScriptableObjects";
    ...
    private void OnEnable()
    {
        ...
        dataResetter = (MONO_DataResetter)target;

        // If the array is null, initialise it to prevent null refs.
        if (dataResetter.SOBJ_resettableScriptableObjects == null)
        {
            dataResetter.SOBJ_resettableScriptableObjects = new SOBJ_ResettableScriptableObject[0];
        }
    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        /* Create a button with a '+' and if it's clicked,
        ...
        /* Create a button with a '-' and if it's clicked remove
         * the last element of the array. Note that if the last
         * element is not null calling DeleteArrayElementAtIndex will make it null.
         */ 
        if (GUILayout.Button("-", GUILayout.Width(buttonWidth)))
        {
