    ...
    public void Save()
    {
        ...
        if (test != null && tes2t != null)
        {
            ...
            for (int i = 0; i < tats.Length; i++)
            {
                ...
                {
                    Fungus.Variable temp = tats[i].GetVariable(data.data[i].keys[v]);
                    ...
                    if (temp2 == typeof(int))
                    {
                        data.data[i].values[v].innt = tats[i].GetIntegerVariable(data.data[i].keys[v]);
                        data.data[i].values[v].isInt = true;
                    }
                    else
                    {
                        data.data[i].values[v].bol = tats[i].GetBooleanVariable(data.data[i].keys[v]);
                    }
                }
            }
                    ...
                    if (flow != null)
                    {
                        ...
                        for (int v = 0; v < flow.GetVariableNames().Length; v++)
                        {
                            ...
                            if (data.data[i].values[i].isInt)
                                {
                                    Debug.Log(data.data[i].keys[v] + "  Value   " + data.data[i].values[v].innt);
                                    flow.SetIntegerVariable(data.data[i].keys[v], data.data[i].values[v].innt);
                            }
                            else
                            {
                                    Debug.Log(data.data[i].keys[v] + "  Value   " + data.data[i].values[v].bol);
                                    flow.SetBooleanVariable(data.data[i].keys[v], data.data[i].values[v].bol);
                            }
